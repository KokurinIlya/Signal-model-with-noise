...
namespace ITSignal
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                points[i] = amplitude1 * Math.Sin(2 * pi * F1 * dt * i + Fi1) + amplitude2 * Math.Sin(2 * pi * F2 * dt * i + Fi2) + amplitude3 * Math.Sin(2 * pi * F3 * dt * i + Fi3);
                chart1.Series["Series1"].Points.AddXY(i/Fd, points[i]);
                chart5.Series["Series1"].Points.AddXY(i/Fd, points[i]);
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int j = 0; j < 12; j++)
                {
                    g += Convert.ToDouble(rnd.Next(-10, 10) / 100.0);
                    Console.WriteLine(g);
                }
                ArrayNoise_old[i] = g;
                Eg += (g) * (g);
            ...
            for (int i = 0; i < n; i++)
            {
                chart4.Series["Series1"].Points.AddXY(i/Fd, xArrayNoised[i]);
            }
            Complex[] CompArray;
            if (Math.Log(n, 2) % 1 != 0)
            {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                chart5.Series["Series2"].Points.AddXY(i/Fd, CompArray[i].Real);
